...
function changeMouseType(val) {
    if (val === "select") {
        ...
        if (!mouse.classList.contains("mouse-active")) {
            document
                .getElementById("mouse-edit")
                .classList.remove("mouse-active");
    ...
    document
        ...
        .forEach((block, index) => {
            ...
            block.addEventListener("click", function (event) {
                ...
                grid.style.display =
                    grid.style.display === "none" ? "flex" : "none"; // Show the grid when the block is clicked.
            });
        });
...
function importSVGToContent() {
    ...
    const svgBlocks = tdBlock.getElementsByClassName("svg-block");
    while (svgBlocks.length > 0) {
        svgBlocks[0].parentNode.removeChild(svgBlocks[0]);
    }
